  ...
  if (postdata.title) {
    utils.downloadHTMLfromBakaTsuki(postdata.title, function(jsondata) {
      ...
      if (jsondata) {
        ...
        data.sections = []
        var status = $("table:contains('Project')")
          .text()
        ...
        if (data.sections.length > 0) {
          ...
          var firstbook = one_off
            ? data.title.replace(/_/g, ' ')
            : data.sections[0].books[0]
          if (firstbook) {
            var volheading = $(
          ...
          for (var serieskey in data.sections) {
            for (var volumekey in data.sections[serieskey].books) {
              ...
              var heading = $(
                ":header:contains('" +
                  data.sections[serieskey].books[volumekey].title.match(
                    /[A-Za-z\d\s\:]+/gi
                  )[0] +
                  "')"
              ).first()
              ...
              headinglinks.each(function() {
                ...
                if (
                  ...
                  data.sections[serieskey].books[volumekey].chapters.push(
                    chapterdata
                  )
                }
              })
            ...
            if (data.sections[serieskey].books.length < 1) {
              var walker = $(
                ":header:contains('" + data.sections[serieskey].title + "')"
              ).nextUntil($(':header'))
              var chapterlinks = walker.find('a')
              chapterlinks.each(function() {
                if (
              ...
              for (var bookind in booklist) {
                for (var chapterind in booklist[bookind].chapters) {
                  ...
                  if (ele.match(/chapter/i)) {
                    data.sections[serieskey].books[bookind].chapters[
                      chapterind
                    ].title =
                      holeid + ':' + ele
                  }
        ...
        if (postdata.volumeno) {
          for (var serieskey in data.sections) {
            ...
            for (var volumekey in data.sections[serieskey].books) {
              ...
              volume_match = data.sections[serieskey].books[
                volumekey
              ].title.match(/\w+ ?\d+/gi)
        ...
        if (one_off) {
          data.sections.map(function(ele) {
